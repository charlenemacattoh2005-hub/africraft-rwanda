  ...
  return (
    <>
      ...
      <div className="admin-page-header">
        ...
        <p className="admin-page-sub">Manage and update all customer orders.</p>
      </div>

      <div className="admin-filter-bar">
        <input className="input" style={{ maxWidth: 260 }} placeholder="Search by order ID or customer…"
          value={search} onChange={e => setSearch(e.target.value)} />
        <select className="admin-filter-select" value={filter} onChange={e => setFilter(e.target.value)}>
          <option value="All">All statuses</option>
          {STATUSES.map(s => <option key={s}>{s}</option>)}
        </select>
        <span className="admin-toolbar-count">{filtered.length} orders</span>
      </div>
                ...
                <tr key={o._id}>
                  ...
                  <td>
                    <div className="admin-customer-cell">
                      <span className="admin-customer-name">{o.customer?.fullName || '—'}</span>
                      <span className="admin-customer-email">{o.customer?.email || ''}</span>
                    </div>
                  </td>
